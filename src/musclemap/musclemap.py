...
def main():
    parser = argparse.ArgumentParser(description="calculate B1, fat fraction, "
                                                 "magnetisation transfer "
                                                 "ratio or T2 maps")

    parser.add_argument("-o", help="output folder for results (defaults are "
                                   "./b1, ./ff, ./mtr, ./mtr-b1 "
                                   " or ./t2",
                        metavar='output_folder', type=pathlib.Path)

    parser.add_argument("-r", help="register images to first "
                                   "source file",
                        action='store_true')

    parser.add_argument("-m", help="filename of mask images",
                        metavar='mask')

    parser.add_argument("-c", help="crop images (indexing starts with 0 not 1,"
                                   "inputting -1 for a size will set it to the "
                                   "full image extent for that dimension)",
                        type=int, nargs=6,
                        metavar=('xmin', 'xsize', 'ymin', 'ysize', 'zmin',
                                 'zsize'))

    parser.add_argument("-k", help="keep intermediate images",
                        action='store_true')

    parser.add_argument("-v", help="view results in FSLeyes",
                        action='store_true')
    ...
    subparsers = parser.add_subparsers(title="algorithm",
                                       dest="algorithm", required=True)

    parser_b1 = subparsers.add_parser('b1', help="calculate B1 map from "
    ...
    parser_ff = subparsers.add_parser('ff', help='ff help',
                                      description="calculate fat fraction maps "
                                                 "from three-point Dixon data")

    parser_mtr = subparsers.add_parser('mtr', help='mtr help',
                                       description=
                                       "calculate magnetisation transfer "
                                       "ratio (MTR) map from MT on and off "
                                       "data")

    parser_mtr_b1 = subparsers.add_parser('mtr-b1', help='mtr-b1 help',
                                          description=
                                          "calculate magnetisation transfer "
                                          "ratio (MTR) map from MT on and off "
    ...
    parser_t2 = subparsers.add_parser('t2', help='calculate T2 map from double-'
                                                 'echo spin-echo data')

    parser_b1.add_argument("fa60", help="60 degree flip-angle spin-echo image",
                           type=pathlib.Path)

    parser_b1.add_argument("fa120", help="120 degree flip-angle spin-echo "
                                         "image",
                           type=pathlib.Path)

    parser_ff.add_argument("m_1", help="out-of-phase fat-water magnitude image"
                                       " (e.g. TE 3.45 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("phi_1", help="out-of-phase fat-water phase image "
                                         "(e.g. TE 3.45 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("m0", help="in-phase fat-water magnitude image "
                                      "(e.g. TE 4.60 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("phi0", help="in-phase fat-water phase image "
                                        "(e.g. TE 4.60 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("m1", help="out-of-phase fat-water magnitude image "
                                      "(e.g. TE 5.75 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("phi1", help="out-of-phase fat-water phase image "
                                        "(e.g. TE 5.75 ms at 3 T)",
                           type=pathlib.Path)

    parser_ff.add_argument("scanner", metavar='scanner', type=str,
                           choices=['ge', 'siemens'],
                           help='scanner type, chose from {%(choices)s}')

    parser_ff.add_argument("-s", help="separate images in x-direction during"
                                      " phase unwrapping (recommended for "
                                      "legs)", action='store_true')

    parser_ff.add_argument("-nb", help="calculate noise-bias corrected "
                                       "fat-fraction map - see Liu et al. "
                                       "MRM 2007", action='store_true')

    parser_mtr.add_argument("mt_on", help="MT on magnitude image",
                            type=pathlib.Path)

    parser_mtr.add_argument("mt_off", help="MT off magnitude image",
                            type=pathlib.Path)

    parser_mtr_b1.add_argument("mt_on", help="MT on magnitude image",
                               type=pathlib.Path)

    parser_mtr_b1.add_argument("mt_off", help="MT off magnitude image",
                               type=pathlib.Path)

    parser_mtr_b1.add_argument("fa60", help="60 degree flip-angle image (for "
                                            "B1 inhomogeneity correction)",
                               type=pathlib.Path)

    parser_mtr_b1.add_argument("fa120", help="120 degree flip-angle image (for "
                                             "B1 inhomogeneity correction)",
                               type=pathlib.Path)

    parser_mtr_b1.add_argument("ref", help="reference image used for masking "
    ...
    parser_t2.add_argument("te1", help="TE of echo 1 in ms (e.g. 16)",
                           type=float)

    parser_t2.add_argument("te2", help="TE of echo 2 in ms (e.g. 56)",
                           type=float)

    parser_t2.add_argument("ref", help="reference image used for registration"
    ...
    if args.algorithm == 'b1':
        ...
        fp_check_dict = {'mt_on_fp': args.mt_on,
                         'mt_off_fp': args.mt_off,
                         'fa60_fp': args.fa60,
                         'fa120_fp': args.fa120,
                         'res_ref_fp': args.ref}
        ...
        if any((args.r, args.c, args.m)):
            sys.stderr.write('* optional arguments -r, -c or -m cannot be used'
        ...
        fp_dict = {'e1_fp': args.e1,
                   'e2_fp': args.e2}

        ref_fp = fp_dict['e1_fp']

        fp_check_dict = {'e1_fp': args.e1,
                         'e2_fp': args.e2,
                         'res_ref_fp': args.ref}
    ...
    if args.r:
        ...
        if args.algorithm == 'ff':
            fp_dict, to_delete = preproc.register_dixon(fp_dict, out_dir,
                                                        to_delete, fsldir,
                                                        args.quiet)
        else:
            fp_dict, to_delete = preproc.register(fp_dict, ref_fp, out_dir,
                                                  to_delete, fsldir, args.quiet)

    if args.m:
        if not args.quiet:
            print('** masking with fslmaths')

        fp_dict, to_delete = preproc.mask(fp_dict, args.m, out_dir, to_delete,
                                          fsldir, args.quiet)
    ...
    if args.c:
        ...
        fp_dict, to_delete = preproc.crop(fp_dict, args.c, out_dir, to_delete,
                                          fsldir, args.quiet)
    ...
    if args.algorithm == 'b1':
        ...
        map_fp, to_delete = ff.process_ff(fp_dict, out_dir, to_delete, fsldir,
                                          args.nb, args.scanner, args.s,
                                          args.quiet)
    elif args.algorithm == 'mtr':
        if not args.quiet:
        ...
        map_fp, to_delete = mtr_b1.process_mtr_b1(fp_dict,
                                                  args.fa60, args.fa120,
                                                  args.ref, out_dir, to_delete,
                                                  fsldir, args.b1pcf,
                                                  args.quiet)
        ...
        map_fp, delete_list = t2.process_t2(fp_dict, args.te1, args.te2,
                                            args.ref, out_dir, to_delete,
                                            fsldir, args.quiet)
