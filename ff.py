...
def unwrap(phim_fp, phim_shape, m0_fp, out_dir, to_delete, split, fsldir,
           quiet=True):
    ...
    if split:
        ...
        for side in ['left', 'right']:
            ...
            prelude_cmd = [str(fsldir / 'bin' / 'prelude'),
                           '-p', str(phim_lr_fp),
                           '-a', str(m0_lr_fp),
                           '-o', str(phim_lr_uw_fp)]
        ...
        fslmaths_cmd = [str(fsldir / 'bin' / 'fslmaths'),
                        str(phim_lr_uw_fp_list[0]), '-add',
                        str(phim_lr_uw_fp_list[1]), str(phim_uw_fp),
                        '-odt', 'float']
...
def process_ff(fp_dict, out_dir, to_delete, fsldir, noise_bias, scanner, split,
               quiet=True):
    ...
    phim_uw_fp, to_delete = unwrap(phim_fp, np.shape(phim), fp_dict['m0_fp'],
                                   out_dir, to_delete, split, fsldir, quiet)
